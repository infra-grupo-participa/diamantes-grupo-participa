 ...
 *
 * Porte fiel de portal/assets/briefing-templates.js (lógica pura, sem DOM).
 *
 * Cada campo pode ter:
...
export type Scope = 'base' | 'project';
...
/** Valor esperado de um campo do tipo 'card'. */
...
/** Mapa plano de respostas (id do campo -> valor). */
export type BriefingAnswers = Record<string, unknown>;

/** Estrutura de briefing de projeto: bloco geral + respostas por serviço. */
...
export interface ProjectBriefingValidationResult {
  valid: boolean;
...
export function getBriefingRequiredFields(serviceType: string): BriefingField[] {
  const tpl = BRIEFING_TEMPLATES[serviceType];
  if (!tpl) return [];
...
export function validateBriefing(
  ...
  const required = getBriefingRequiredFields(serviceType);
  ...
  for (const field of required) {
    ...
    if (field.type === 'card') {
      const card = val as CardValue | null | undefined;
      if (!card || !card.brand || !card.last4 || card.last4.length < 4 || !card.expiry) {
...
export const BRIEFING_ACTIVE_SERVICES: string[] = ['anuncios_pagos', 'edicao_video', 'paginas', 'automacao'];

// Rótulos curtos por serviço (para UI).
export const BRIEFING_SERVICE_LABELS: Record<string, string> = {
...
export const BRIEFING_SECTION_SCOPE: Record<string, Record<string, Scope>> = {
  anuncios_pagos: {
...
export const BRIEFING_FIELD_SCOPE: Record<string, Record<string, Scope>> = {
  // ex.: anuncios_pagos: { seminar_date: 'project' }
};

// Bloco geral do evento (transversal a todos os serviços) — vive em briefing.general
export const GENERAL_PROJECT_FIELDS: BriefingField[] = [
...
export function getFieldScope(service: string, sectionId: string, field: BriefingField | null | undefined): Scope {
  if (field && field.scope) return field.scope;
  const fc = (BRIEFING_FIELD_SCOPE[service] || {})[(field && field.id) as string];
  if (fc) return fc;
  return (BRIEFING_SECTION_SCOPE[service] || {})[sectionId] || 'project';
}

// Retorna as seções de um serviço filtradas por scope (seções vazias são omitidas).
export function getServiceSections(service: string, scope: Scope): BriefingSection[] {
  const tpl = BRIEFING_TEMPLATES[service];
  if (!tpl) return [];
  return tpl.sections
    .map((sec) => ({ ...sec, fields: sec.fields.filter((f) => getFieldScope(service, sec.id, f) === scope) }))
    .filter((sec) => sec.fields.length > 0);
}

export function getBaseSections(service: string): BriefingSection[] {
  return getServiceSections(service, 'base');
}

export function getProjectSections(service: string): BriefingSection[] {
  return getServiceSections(service, 'project');
}

export function getGeneralFields(): BriefingField[] {
  return GENERAL_PROJECT_FIELDS;
}

// Verifica se um campo (red) está vazio, respeitando dependsOn/readonly/tipo.
export function fieldIsEmpty(field: BriefingField, answers: BriefingAnswers): boolean {
  ...
  if (field.type === 'card') {
    const card = val as CardValue | null | undefined;
    return !card || !card.brand || !card.last4 || card.last4.length < 4 || !card.expiry;
  }
...
export function validateBaseAccess(service: string, answers?: BriefingAnswers): BriefingValidationResult {
  const missing: string[] = [];
  getBaseSections(service).forEach((sec) =>
    sec.fields.forEach((f) => {
      if (f.priority === 'red' && fieldIsEmpty(f, answers || {})) missing.push(f.id);
    }),
  );
  return { valid: missing.length === 0, missing };
}
...
export function validateProjectBriefing(
  ...
  (services || []).forEach((svc) => {
    ...
    getProjectSections(svc).forEach((sec) =>
      sec.fields.forEach((f) => {
        if (f.priority === 'red' && fieldIsEmpty(f, ans)) missing.push({ scope: svc, field: f.id });
      }),
    );
  });
  return { valid: missing.length === 0, missing };
